...
public:
    ...
    run() override
    {
        ...
        BEAST_EXPECT(hist.min() == 0);
        BEAST_EXPECT(hist.max() == 0);
        BEAST_EXPECT(hist.avg() == 0);
        BEAST_EXPECT(hist.percentile(0.0f) == hist.min());
        BEAST_EXPECT(hist.percentile(0.5f) == 0);
        BEAST_EXPECT(hist.percentile(0.9f) == 0);
        BEAST_EXPECT(hist.percentile(1.0f) == hist.max());

        hist.insert(1);

        BEAST_EXPECT(hist.size() == 1);
        BEAST_EXPECT(hist.numBins() == 1);
        BEAST_EXPECT(hist.min() == 1);
        BEAST_EXPECT(hist.max() == 1);
